...
class App extends Component {
  ...
  startGameAgain = () => {
    ...
    this.timerId = setInterval(() => {
      ...
      if (timer === 0) {
        this.onClearInterval()
        this.setState({isGameInProgress: false, imageUrlIndex: 0})
      }
      this.setState(prevState => ({timer: prevState.timer - 1}))
    }, 1000)
  }
  ...
  changeImage = checkId => {
    ...
    const {id} = imagesList[imageUrlIndex]
  ...
  rendersItemsContainer = () => {
    const {activeTab, imageUrlIndex} = this.state
    return (
      <div className="game-container">
        <div className="game-image-container">
          <img
            src={imagesList[imageUrlIndex].imageUrl}
            alt="match"
            className="fruit-main-image"
          />
        </div>
        <ul className="tabs-container">
          {tabsList.map(tab => (
            <TabItem
              tabDetails={tab}
              key={tab.tabId}
              isActiveTab={activeTab === tab.tabId}
  ...
  renderImagesList = () => {
    ...
    const filterImageLists = imagesList.filter(
      item => item.category === activeTab,
    )

    console.log(filterImageLists)

    return (
      <ul className="images-lits-container">
        {filterImageLists.map(image => (
          <ImageItem
            image={image}
      ...
      <div className="game-container">
        <div className="result-container">
          ...
          >
            ...
            />
            <p className="text-play">PLAY AGAIN</p>
          </button>
        </div>
      </div>
    )
